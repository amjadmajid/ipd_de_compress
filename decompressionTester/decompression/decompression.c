...
unsigned int * WP2 = (unsigned int *) 0x18b2;	// WP1+1;
unsigned char MWP = 0 ;
...
static unsigned char *init_flag = 	 ( unsigned char *) 0x1950; //TODO make sure this is initially 0xff
...
void decompression(){
	...
	while(1){
		...
		if ( ((unsigned int) *WP1 -  0x6600) >= *trmi ){
			break;
		}
	}
...
void checkAndMove(){
	...
	if(*MPsP == 0x00) 		// normal transaction procedure
	{
		...
		*RP1 += MRP;
		...
		if(MWP == 0 && MRP == 0 && Buf == 0 && MBP == 0)  // if wisp recover from die during the state-less operation stage
		{
			Buf = *HBuf1;
			MBP = *BP1 ;
			MWF	= *WF1 ;
	...
	// *MPsP= 0x00; 		// the tansition is finished AND remove all the above
}

void loadByte(void)
{
	//	Buf |= *( (unsigned char *)(*RP1+xr)) << (8 - MBP) ;
	Buf |= *( (unsigned char *)(*RP1)) << (8 - MBP) ;
	MRP++;
}
